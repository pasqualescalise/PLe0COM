...
class Parser:
    ...
    def initialize_heap_memory(self):
        ir.DataSymbolTable.add_data_symbol(ir.ArrayType(None, [ir.HEAP_SIZE], ir.TYPENAMES['byte']), name="heap")
        ir.DataSymbolTable.add_data_symbol(ir.PointerType(ir.TYPENAMES['int']), value="heap", name="brk")
    ...
    def statement(self, symtab):
        if self.accept('beginsym'):
            ...
            return ast.NewStat(children=[target], expr=size_expr, symtab=symtab)
